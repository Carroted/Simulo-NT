...
interface SimuloSpringDesc {
    ...
    getBodyAPosition: () => Rapier.Vector3;
    getBodyBPosition: () => Rapier.Vector3;

    getBodyARotation: () => Rapier.Rotation;
    getBodyBRotation: () => Rapier.Rotation;

    getBodyAVelocity: () => Rapier.Vector3;
    getBodyBVelocity: () => Rapier.Vector3;

    getBodyAAngularVelocity: () => Rapier.Vector3;
    getBodyBAngularVelocity: () => Rapier.Vector3;

    applyBodyAImpulse: (impulse: Rapier.Vector3, worldPoint: Rapier.Vector3) => void;
    applyBodyBImpulse: (impulse: Rapier.Vector3, worldPoint: Rapier.Vector3) => void;

    localAnchorA: Rapier.Vector3;
    localAnchorB: Rapier.Vector3;
...
class SimuloSpring {
    ...
    private server: SimuloPhysicsServerRapier2D;
    constructor(server: SimuloPhysicsServerRapier2D, desc: SimuloSpringDesc, id: string) {
        this.id = id;
        this.desc = desc;
        this.server = server;
    }
...
class SimuloPhysicsServerRapier2D implements SimuloPhysicsServer {
    type: "2d" | "3d" = "2d";
    ...
    addSpring(spring: {
        ...
        let springDesc: SimuloSpringDesc = {
            applyBodyAImpulse: (impulse: Rapier.Vector3, worldPoint: Rapier.Vector3) => {
                if (spring.objectA) {
            ...
            applyBodyBImpulse: (impulse: Rapier.Vector3, worldPoint: Rapier.Vector3) => {
                if (spring.objectB) {
            ...
            getBodyARotation: () => {
                ...
                return RAPIER.RotationOps.identity();
            },
            getBodyBRotation: () => {
                if (spring.objectB) {
                    if (spring.objectB.reference instanceof RAPIER.RigidBody) {
            ...
            localAnchorA: new RAPIER.Vector3(spring.localAnchorA.x, spring.localAnchorA.y, spring.localAnchorA.z),
            localAnchorB: new RAPIER.Vector3(spring.localAnchorB.x, spring.localAnchorB.y, spring.localAnchorB.z),
        ...
        return {
            ...
            setLocalAnchorA: (localAnchorA: { x: number, y: number, z: number }) => {
                springDesc.localAnchorA = new RAPIER.Vector3(localAnchorA.x, localAnchorA.y, localAnchorA.z);
            },
            setLocalAnchorB: (localAnchorB: { x: number, y: number, z: number }) => {
                springDesc.localAnchorB = new RAPIER.Vector3(localAnchorB.x, localAnchorB.y, localAnchorB.z);
            },
            reference: id,
        };
    }
    ...
    async init() {
        ...
        let gravity = new RAPIER.Vector3(this.gravity.x, this.gravity.y);
    ...
    getObjectAtPoint(point: { x: number, y: number, z: number }): SimuloObject | null {
        if (!this.world) { throw new Error('init world first'); }
        let vec = new RAPIER.Vector3(point.x, point.y);
    ...
    normalize(v: Rapier.Vector3): Rapier.Vector3 {
        let len = Math.sqrt(v.x ** 2 + v.y ** 2);
        if (len === 0) return new RAPIER.Vector3(0, 0);
        return new RAPIER.Vector3(v.x / len, v.y / len);
    }

    sub(a: Rapier.Vector3, ...others: Rapier.Vector3[]): Rapier.Vector3 {
        let x = a.x;
        let y = a.y;
        ...
        return new RAPIER.Vector3(x, y);
    }

    magnitude(v: Rapier.Vector3): number {
        return Math.sqrt(v.x ** 2 + v.y ** 2);
    }

    distance(a: Rapier.Vector3, b: Rapier.Vector3): number {
        return this.magnitude(this.sub(a, b));
    }

    multiply(v: Rapier.Vector3, s: number): Rapier.Vector3 {
        return new RAPIER.Vector3(v.x * s, v.y * s);
    }

    vecToString(v: Rapier.Vector3): string {
        return `(${v.x}, ${v.y})`;
    }

    add(a: Rapier.Vector3, ...others: Rapier.Vector3[]): Rapier.Vector3 {
        let x = a.x;
        let y = a.y;
        others.forEach((other) => {
            x += other.x;
    ...
    dot(a: Rapier.Vector3, b: Rapier.Vector3): number {
        return a.x * b.x + a.y * b.y;
    }

    cross(a: Rapier.Vector3, b: Rapier.Vector3): Rapier.Vector3 {
        return new RAPIER.Vector3(a.x * b.y, -a.y * b.x);
    }
    crossZV(z: number, v: Rapier.Vector3): Rapier.Vector3 {
        return new RAPIER.Vector3(-z * v.y, z * v.x);
    }

    /** Convert a world point to a local point */
    getLocalPoint(bodyPosition: RAPIER.Vector3, bodyRotation: number, worldPoint: RAPIER.Vector3) {
        ...
        return new RAPIER.Vector3(localX, localY);
    }

    /** Convert a local point to a world point */
    getWorldPoint(bodyPosition: RAPIER.Vector3, bodyRotation: number, localPoint: RAPIER.Vector3) {
        const cos = Math.cos(bodyRotation);
        const sin = Math.sin(bodyRotation);
        ...
        return new RAPIER.Vector3(worldX, worldY);
    }
    ...
    getLocalObjectPoint(object: SimuloObject, worldPoint: { x: number; y: number; z: number; }): { x: number; y: number; z: number; } {
        ...
        let localPoint = this.getLocalPoint(body.translation(), body.rotation(), new RAPIER.Vector3(worldPoint.x, worldPoint.y));
    ...
    getWorldObjectPoint(object: SimuloObject, localPoint: { x: number; y: number; z: number; }): { x: number; y: number; z: number; } {
        ...
        let worldPoint = this.getWorldPoint(body.translation(), body.rotation(), new RAPIER.Vector3(localPoint.x, localPoint.y));
        return { x: worldPoint.x, y: worldPoint.y, z: 0 };
    }
}

export default SimuloPhysicsServerRapier2D;
